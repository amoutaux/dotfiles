...
def print_tags(i_dict: dict[str, timedelta]) -> None:
    ...
    maxlen_key = max(len(max(i_dict, key=len)), len("Total"))
    maxlen_time = max(
    ...
    for key in sorted(i_dict):
        percentage = f"%{round(i_dict[key] / total_time * 100)}"
        print(
            key.ljust(maxlen_key),
            "--",
...
def convert_timestamps(
    data: list[dict[str, str]], rep_start: str, rep_end: str
) -> None:
    """
